...
| **el encargo a FOL** | `Calculus/Subst.lean` y `fdepth` duplicados | ✅ **CONTESTADO dos veces el 2026-09-23** (PRF-047 y [PRF-048](RESPUESTA-FOL-2026-09-23b.md)). **§3 ACEPTADO** ⇒ planificar la **retirada de `fdepth`**. ⬜ **§2 REABRE**: FOL va a por la propiedad de disyunción y `Subst.lean` pasa a tener consumidor allí ⇒ **recomendamos ahora que ENTRE** (esta mañana, sin consumidor, recomendamos lo contrario). Decisión del propietario |
| **la propuesta (C)** | que `Subst` + `DerivesI` + `Slash` bajen a FOL | ⬜⬜⬜ **decisión del propietario, y FOL no se sella hasta que contestemos** ([PRF-049](RESPUESTA-FOL-2026-09-23c.md)). ✅ La dirección es correcta. ⚠️ Pero son **7 módulos y 114 de las 442 decls**, no 3 — y ⛔ `Collapse.lean:61` abre RPP, que importa FOL: **ese `open` en FOL sería un CICLO**. Recomendado reparto en dos tiempos |
| **la DP para el fragmento de FOL** | `Derives₀ᵢ` = `Derives₀` menos los tres clásicos | 🏁 **YA EXISTE y es nuestro `Derivesᵢ`** — mismos 18 constructores, con `derivesI_to_derives0` y la DP probada. Centinela en `sondeos/fragmento_intuicionista.lean`. ⚠️ Lo que no se traslada gratis: su `Derives₀` es polimórfico en `Sym` y el nuestro no (PRF-048 §7) |
